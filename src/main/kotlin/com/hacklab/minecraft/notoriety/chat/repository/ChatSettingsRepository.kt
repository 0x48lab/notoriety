...
class ChatSettingsRepository(private val databaseManager: DatabaseManager) {
    ...
    fun save(settings: PlayerChatSettings) {
        databaseManager.provider.useConnection { conn ->
            val sql = """
                INSERT OR REPLACE INTO player_chat_settings (player_uuid, chat_mode, romaji_enabled)
                VALUES (?, ?, ?)
            """.trimIndent()
            conn.prepareStatement(sql).use { stmt ->
    ...
    fun updateChatMode(playerUuid: UUID, mode: ChatMode) {
        databaseManager.provider.useConnection { conn ->
            val sql = """
                INSERT OR REPLACE INTO player_chat_settings (player_uuid, chat_mode, romaji_enabled)
                VALUES (?, ?, COALESCE((SELECT romaji_enabled FROM player_chat_settings WHERE player_uuid = ?), FALSE))
            """.trimIndent()
            conn.prepareStatement(sql).use { stmt ->
                stmt.setString(1, playerUuid.toString())
    ...
    fun updateRomajiEnabled(playerUuid: UUID, enabled: Boolean) {
        databaseManager.provider.useConnection { conn ->
            val sql = """
                INSERT OR REPLACE INTO player_chat_settings (player_uuid, chat_mode, romaji_enabled)
                VALUES (?, COALESCE((SELECT chat_mode FROM player_chat_settings WHERE player_uuid = ?), 'LOCAL'), ?)
            """.trimIndent()
            conn.prepareStatement(sql).use { stmt ->
                stmt.setString(1, playerUuid.toString())
